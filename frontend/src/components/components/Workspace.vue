    ...
    <div class="day-container">
        <div class="menu-bar">
            <div class="menu-item" v-for="(day, index) in days" :key="index">
                ...
                <div class="drop-zone" :dropzone=true :day="day" @dragover.prevent @drop.prevent
                    @drop="addRecipe($event, day)">
                    <ul class="recipe-day-list">
                        <li v-for="(recipe, index) in weeklyList[day]"
                            :key="index"
                            >
                            <p>{{ recipe.name }}</p>
                            <button @click="deleteRecipe(day, recipeIndex)" class="delete-recipe-button">x</button>
                        </li>
                    </ul>                  
                </div>
            </div>
    ...
    methods: {
        addRecipe(event, day) {
            ...
            if (recipeData) {
                ...
                console.log("All recipes saved in "+day+": " +JSON.stringify(this.weeklyList[day], null, 2));
            }
        },
